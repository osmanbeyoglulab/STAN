...
from ..util.spatrafact_util import make_kernel, adata2pd
...
def plot_kernel_samples(X, n_samples, std=2, im_feat_weights=0.1):
    kernel=make_kernel(X, n_samples, std=std, im_feats_weight=im_feat_weights)
...
def plot_hist_feats(adata, n_feats=None):
    ...
    for i in range(n_feats):
        plt.subplot(int(np.ceil(n_feats/2)), 4, i+1)
...
def dotplot(data, x, y, color, size, sizes=(0,200), colors=(-0.5,0.5), dpi=300):
    ...
    f=sns.clustermap(data.pivot(index=y, columns=x, values=color),figsize=(1,1))
